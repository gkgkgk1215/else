...
class MappingC2R():
    ...
    """
    def __init__(self, filename_mapping_table):
        # checkerboard dimension
        self.__row = 6
        self.__col = 8

        # checkerboard corners
        filename = 'calibration_files/corners_8x6.npy'
        self.__cb_corners = np.load(filename).reshape(self.__row, self.__col, 2)
        # filename = '../calibration_files/checker_board_inclined.npy'
        # self.__cb_corners = np.load(filename).reshape(self.__row, self.__col, 2)

        # mapping table (cb corners <-> robot)
        filename = filename_mapping_table
        self.__mapping_table = self.load_mapping_table(filename, self.__row, self.__col)
        print self.__mapping_table

    def load_mapping_table(self, filename, row_board, col_board):
        data_default = np.loadtxt(filename, delimiter=',')
        mapping_table = np.zeros((row_board, col_board, data_default.shape[1]-2))
    ...
    def transform_pixel2robot(self, input_pixel):
        ...
        y1 = pts_dst[0][1]  # =0
        y2 = pts_dst[1][1]  # =100
        fQ11 = self.__mapping_table[arg[0][0], arg[0][1]]
        fQ12 = self.__mapping_table[arg[1][0], arg[1][1]]
        fQ21 = self.__mapping_table[arg[3][0], arg[3][1]]
        fQ22 = self.__mapping_table[arg[2][0], arg[2][1]]
        # print x,y
        # print x1,x2
        # print y1,y2
        # print fQ11, fQ12, fQ21, fQ22
        output = self.bilinear_interpolation((x,y), (x1,x2), (y1,y2), (fQ11,fQ12,fQ21,fQ22))
        return output

if __name__ == '__main__':
    mapping = MappingC2R('../calibration_files/mapping_table_PSM2')
    input_pixel = [104,   94]
    input_pixel = [  76.96347809,  68.91957855]
    input_pixel = [442,334]
    output = mapping.transform_pixel2robot(input_pixel)
